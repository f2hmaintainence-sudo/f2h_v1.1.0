  ...
  async saveOffer(body: any, adminId: string) {
    try {
      ...
      const insertData: Record<string, any> = {
        ...
        action_value: body.action_value?.trim() || (body.action_type === 'CATEGORY' ? body.category_id : null) || null,
        category_id: body.category_id?.trim() || null,
        banner_type: body.banner_type || 'home_carousel',
        is_popup: body.is_popup !== undefined ? Boolean(body.is_popup) : false,
