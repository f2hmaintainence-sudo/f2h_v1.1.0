...
export class DeliveryRunService {
  ...
  private async resolvePartnerUserInfo(partnerIdOrUserId?: string | null): Promise<{ userId: string; partnerName: string } | null> {
    ...
    try {
      ...
      return {
        userId: rows[0].delivery_partner_id || partnerIdOrUserId,
  ...
  async createOptimizedRuns(date?: string, branchId?: string, slot?: string, adminId?: string) {
    const targetDate = date || todayIST();
    this.developer.debug('createOptimizedRuns parameters', { date, branchId, slot, targetDate, adminId });
        ...
        await this.db.query(
          ...
          [
            ...
            JSON.stringify({
              ...
              run_ids: createdRuns.map((r) => r.run_id),
            }),
          ],
        );
      } catch (_) { }
        ...
        SELECT
          ...
          (
            SELECT json_agg(json_build_object(
              ...
              'customer_name', COALESCE(NULLIF(TRIM(ca.contact_name), ''), NULLIF(TRIM(o.customer_name), ''), NULLIF(TRIM(COALESCE(u.first_name, '') || ' ' || COALESCE(u.last_name, '')), ''), u.user_name, 'Customer'),
  ...
  async getEligibleTargetRuns(orderId: string) {
    this.developer.debug('DeliveryRunService.getEligibleTargetRuns called', { orderId });
    try {
      const orderRes = await this.db.query(
        `SELECT o.order_id, o.customer_id,
                COALESCE(NULLIF(TRIM(ca.contact_name), ''), NULLIF(TRIM(o.customer_name), ''), NULLIF(TRIM(COALESCE(cu.first_name, '') || ' ' || COALESCE(cu.last_name, '')), ''), cu.user_name, 'Customer') AS customer_name,
                o.address_id,
      ...
      this.developer.error('getEligibleTargetRuns error', { error, orderId });
      throw new BadRequestException(error?.message || 'Failed to fetch eligible target runs');
    }
  }
    ...
    try {
      return await this.db.transaction(async (client) => {
        ...
        for (let i = 0; i < remainingSrcStops.rows.length; i++) {
          await client.query(
            `UPDATE delivery_run_addresses SET sequence_no = $1, updated_at = NOW() WHERE id = $2`,
            [i + 1, remainingSrcStops.rows[i].id],
          );
        }
        ...
        try {
          const stopDetailRes = await client.query<any>(
            `SELECT dra.address_id, dra.customer_id,
                    COALESCE(NULLIF(TRIM(ca.contact_name), ''), NULLIF(TRIM(CONCAT(cu.first_name, ' ', cu.last_name)), ''), cu.user_name, 'Customer') AS customer_name,
                    COALESCE(NULLIF(TRIM(ca.address_line), ''), ca.landmark, 'Customer Address') AS address_line
             FROM delivery_run_addresses dra
             LEFT JOIN customer_addresses ca ON ca.address_id = dra.address_id
        ...
        try {
          const srcPartnerRes = await client.query<any>(
            `SELECT dp.delivery_partner_id,
             ...
             FROM delivery_partners dp
             LEFT JOIN users u ON u.user_id = dp.delivery_partner_id
             WHERE dp.delivery_partner_id = $1 OR u.user_id = $1 LIMIT 1`,
  ...
  async getRunsWithOrders(query: { date?: string; branch_id?: string; slot?: string; status?: string }) {
    try {
      const targetDate = query?.date || todayIST();
      const conditions: string[] = ['dr.run_date = $1', 'dr.deleted_at IS NULL'];
      const params: any[] = [targetDate];
      let pIdx = 2;

      if (query?.branch_id) {
        conditions.push(`dr.branch_id = $${pIdx++}`);
        params.push(query.branch_id);
      }
      if (query?.slot) {
        conditions.push(`dr.delivery_slot = $${pIdx++}`);
        params.push(query.slot);
      }
      if (query?.status) {
        conditions.push(`dr.status = $${pIdx++}`);
          ...
          COALESCE(
            (SELECT json_agg(
              json_build_object(
                ...
                'customer_name', COALESCE(NULLIF(TRIM(ca.contact_name), ''), NULLIF(TRIM(COALESCE(cu.first_name, '') || ' ' || COALESCE(cu.last_name, '')), ''), cu.user_name, 'Customer'),
                ...
                'orders', COALESCE(
                  (SELECT json_agg(
                    json_build_object(
                      'order_id', o.order_id,
                      'customer_id', o.customer_id,
                      'customer_name', COALESCE(NULLIF(TRIM(oca.contact_name), ''), NULLIF(TRIM(o.customer_name), ''), NULLIF(TRIM(COALESCE(ocu.first_name, '') || ' ' || COALESCE(ocu.last_name, '')), ''), ocu.user_name, 'Customer'),
          ...
          COALESCE(
            (SELECT json_agg(
              json_build_object(
                'order_id', o.order_id,
                'customer_id', o.customer_id,
                'customer_name', COALESCE(NULLIF(TRIM(ca.contact_name), ''), NULLIF(TRIM(o.customer_name), ''), NULLIF(TRIM(COALESCE(cu.first_name, '') || ' ' || COALESCE(cu.last_name, '')), ''), o.customer_name, 'Customer'),
                'address_id', o.address_id,
      ...
      return {
        ...
        message: 'Delivery runs with orders fetched successfully',
      };
    } catch (error: any) {
      this.developer.error('getRunsWithOrders error', { error, query });
