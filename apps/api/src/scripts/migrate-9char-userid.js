...
async function run() {
  ...
  console.log('Connected to PostgreSQL database for 9-char User ID migration.');
  ...
  try {
    ...
    // Check existing columns in database to avoid errors on non-existent columns
    for (const item of userColumns) {
      for (const [oldId, newId] of idMap.entries()) {
        ...
        try {
          ...
          // Column or table might not exist in target DB environment, log and continue
    ...
    console.log('SUCCESS: All 13 users & referencing records updated to 9-character User IDs!');

    // Print resulting user list
    const updatedUsers = await client.query('SELECT user_id, email, phone, role_id, referred_by FROM users');
    console.log('Migrated Users List:', updatedUsers.rows);
