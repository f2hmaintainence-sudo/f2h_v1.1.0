...
export default function DispatchPlanningDashboard() {
        ...
        if (res.data?.status && Array.isArray(res.data.data)) {
          ...
          const mapped = sortedDates.map((dateStr) => {
            ...
            if (dateStr === todayStr) label = `Today (${label})`;
              ...
              const yParts = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Kolkata", year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(new Date(new Date().setDate(new Date().getDate() - 1)));
              const yStr = `${yParts.find((p) => p.type === "year")?.value}-${yParts.find((p) => p.type === "month")?.value}-${yParts.find((p) => p.type === "day")?.value}`;
              if (dateStr === yStr) label = `Yesterday (${label})`;
            }
    ...
    api.get<any>("/admin/warehouses/active/list")
      .then((res) => {
        if (res.data?.data) {
          ...
          if (res.data.data.length > 0) setSelectedWarehouse(res.data.data[0].warehouse_id || res.data.data[0].id);
        }
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="space-y-5 p-2 md:p-4 font-sans min-h-screen bg-slate-50/60 pb-16">
      ...
      {/* Breadcrumb */}
      <nav className="flex items-center gap-1.5 text-sm text-gray-500 no-print">
        ...
        <ChevronRight size={14} className="text-gray-300" />
        <span className="font-semibold text-emerald-800">Dispatch Planning</span>
      </nav>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              Dispatch Planning <Sparkles size={16} className="text-[#2E7D32]" />
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">Plan, verify and issue stock for today's delivery runs</p>
          </div>
        </div>
                      ...
                      {filteredPickList.length === 0 ? (
                        ...
                        <tr key={idx} className="hover:bg-slate-50/60 transition-colors group">
                          ...
                          <td className="px-5 py-4 text-center">
                            <span className="inline-flex items-center justify-center px-2.5 py-1 bg-emerald-600 text-white rounded-lg font-black text-[12px]">
                              ×{item.quantity}
                            </span>
                          </td>
                          <td className="px-5 py-4 text-center">
                    ...
                    const renderCard = (plan: DeliveryPartnerPlan) => {
                      ...
                      return (
                        <div key={plan.run_id} id={`rider-card-${plan.run_id}`}
                          ...
                          {/* Card header */}
                          <div
                            onClick={() => setExpandedCards((prev) => ({ ...prev, [plan.run_id]: !prev[plan.run_id] }))}
                          ...
                          >
                            ...
                            <div className="flex flex-wrap items-center gap-2 self-end lg:self-auto">
                              ...
                              {!isDispatched && (
                                <button
                                  onClick={(e) => { e.stopPropagation(); if (!isApproving) handleApproveDispatch(plan); }}
                                  ...
                                  className={`inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold transition-all no-print shadow-sm ${
                                    isApproving
                                      ? "bg-emerald-50 text-emerald-600 cursor-wait border border-emerald-200"
                                      : "bg-[#2E7D32] hover:bg-[#1B5E20] text-white shadow-[#2E7D32]/20 hover:scale-[1.02] active:scale-[0.98]"
                                  }`}>
                                      ...
                                      {totalPages > 1 && (
                                        <div className="flex items-center justify-between pt-2 border-t border-slate-100 bg-white/50 px-2 py-1.5 rounded-xl no-print">
                                          <button
                                            ...
                                            onClick={() => setOrdersPage(prev => ({ ...prev, [plan.run_id]: Math.max(1, currentPage - 1) }))}
                                            className="px-2.5 py-1 text-[10px] font-black text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                                            Previous
                                          </button>
                                          ...
                                          <button
                                            ...
                                            onClick={() => setOrdersPage(prev => ({ ...prev, [plan.run_id]: Math.min(totalPages, currentPage + 1) }))}
                                            className="px-2.5 py-1 text-[10px] font-black text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                                            Next
                                          </button>
                                        </div>
                      ...
                      <>
                        ...
                        {pendingPlans.length > 0 && (
                          <div className="rounded-2xl border border-amber-200/80 overflow-hidden shadow-xs">
                            <button
                              onClick={() => setPendingAccordionOpen((o) => !o)}
                        ...
                        <div className="space-y-3">
                          ...
                          {dispatchedPlans.length === 0 ? (
                            ...
                            // Dynamically group dispatched plans by slot (shift)
                            ...
                            return Object.entries(shiftsMap).map(([shiftName, shiftPlans]) => {
                              // Group shiftPlans by rider name so they are merged into one card per rider per shift
                              ...
                              // Map rider groups to consolidated partner plans
                              const consolidatedRiderPlans: DeliveryPartnerPlan[] = Object.entries(riderGroups).map(([riderName, plansList]) => {
                                const first = plansList[0];
                                
                                // Merge run numbers/IDs
                                ...
                                // Consolidate totals of items (totals key is product_variant_id)
                                const mergedTotals: Record<string, any> = {};
                                plansList.forEach((p) => {
                                  if (p.totals) {
                                    Object.entries(p.totals).forEach(([variantId, item]: [string, any]) => {
                                      if (!mergedTotals[variantId]) {
                                        mergedTotals[variantId] = { ...item };
                                      } else {
                                        mergedTotals[variantId].quantity += item.quantity;
                                        mergedTotals[variantId].orderCount = (mergedTotals[variantId].orderCount || 0) + (item.orderCount || 0);
                                      }
                                    });
                                  }
                                });
                                ...
                                plansList.forEach((p) => {
                                  if (p.orders) {
                                    mergedOrders.push(...p.orders);
                                  }
                                });

                                return {
                                  ...first,
                                  run_number: runNumbers.join(" / "),
                                  run_id: runIds.join("_"), // concatenated unique key
                                  totalOrders,
                                  totalCustomers,
                                  totalProducts,
                                  totalQuantity,
                              ...
                              // Aggregate totals for the shift
                              ...
                              return (
                                <div key={shiftName} className="rounded-2xl border border-emerald-200/80 overflow-hidden shadow-xs">
                                  <button
                                    onClick={() => setExpandedCards((prev) => ({ ...prev, [stateKey]: !prev[stateKey] }))}
                          ...
                          <tr key={item.product_variant_id || idx} className="hover:bg-slate-50/40 transition-colors">
                            ...
                            <td className="px-4 py-3.5 text-center">
                              <div className="inline-flex items-center bg-slate-50 border border-slate-200 rounded-xl overflow-hidden shadow-xs">
                                <button type="button"
                                  onClick={() => setModalItems((prev) => prev.map((mi) => mi.product_variant_id === item.product_variant_id ? { ...mi, loaded_qty: Math.max(0, (mi.loaded_qty || 0) - 1) } : mi))}
                                  className="w-8 h-8 flex items-center justify-center text-slate-600 font-black hover:bg-slate-100 border-r border-slate-200 text-sm active:scale-90 transition-all">
                                  −
                                </button>
                                ...
                                />
                                <button type="button"
                                  onClick={() => setModalItems((prev) => prev.map((mi) => mi.product_variant_id === item.product_variant_id ? { ...mi, loaded_qty: (mi.loaded_qty || 0) + 1 } : mi))}
                                  className="w-8 h-8 flex items-center justify-center text-slate-600 font-black hover:bg-slate-100 border-l border-slate-200 text-sm active:scale-90 transition-all">
                                  +
                                </button>
                              </div>
                            </td>
              ...
              <div className="bg-slate-50/60 border border-slate-200/60 rounded-2xl p-4 space-y-3">
                ...
                <div className="flex flex-col md:flex-row gap-2">
                  <div className="relative flex-1">
                    ...
                    <input type="text" placeholder="Search product or variant to add…"
                    ...
                    {isSearchingVariantDropdownOpen && (
                      <div className="absolute z-50 left-0 right-0 mt-1 max-h-48 overflow-y-auto bg-white border border-slate-200 rounded-xl shadow-xl divide-y divide-slate-50">
                        {availableVariants
                          .filter((v) => `${v.product_name} ${v.variant_name} ${v.unit_value || ""} ${v.unit_type || ""}`.toLowerCase().includes(searchQueryVariant.toLowerCase()))
                          .map((v) => {
