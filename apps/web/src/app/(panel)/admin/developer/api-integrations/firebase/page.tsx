...
// Project     : F2H Fresh
// File        : page.tsx
// Description : Developer API Integrations - Firebase Management Page
//
// ============================================================================
...
import { FirebaseConfigModal } from "@/components/developer/FirebaseConfigModal";
import {
  Layers,
  Plus,
  Search,
...
export default function FirebaseIntegrationsPage() {
  ...
  const fetchConfigs = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get("/admin/developer/api-integrations/firebase");
      ...
      showErrorToast(err?.message || "Failed to load Firebase configurations");
      setConfigs([]);
    } finally {
      setLoading(false);
    }
    ...
    try {
      if (formData.id) {
        const res = await api.put(
          `/admin/developer/api-integrations/firebase/${formData.id}`,
          formData
        );
        if (res.status) {
          showSuccessToast("Firebase configuration updated successfully");
          fetchConfigs();
        } else {
        ...
        const res = await api.post(
          "/admin/developer/api-integrations/firebase",
          formData
        );
        if (res.status) {
          showSuccessToast("Firebase configuration saved successfully");
          fetchConfigs();
        } else {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this Firebase configuration?"))
      return;
    try {
      const res = await api.delete(
        `/admin/developer/api-integrations/firebase/${id}`
      );
      if (res.status) {
  ...
  const handleToggleActive = async (config: any) => {
    try {
      const res = await api.put(
        `/admin/developer/api-integrations/firebase/${config.id}`,
        {
          ...config,
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      <ApiIntegrationsHeader
        ...
        subtitle="Configure Firebase Cloud Messaging (FCM), Push Notification Service Accounts, and mobile app IDs."
        configCount={configs.filter((c) => c.is_active).length}
        secondaryTitle="APPS REGISTERED"
        secondaryCount={configs.length}
        secondarySubtext="Active mobile & web app projects"
        dispatchedCount={4820}
        reliabilityValue="99.99%"
      />
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Push notification service keys &amp; project credentials
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center space-y-3">
          ...
          <h3 className="text-sm font-bold text-slate-700">
            No Firebase Provider configured
          </h3>
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            Click &quot;Add Firebase Config&quot; to register your FCM service account or mobile client settings.
          </p>
        </div>
      ) : (
