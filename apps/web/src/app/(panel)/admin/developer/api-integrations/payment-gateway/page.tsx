...
// Project     : F2H Fresh
// File        : page.tsx
// Description : Developer API Integrations - Payment Gateway Management Page
//
// ============================================================================
...
import { PaymentGatewayConfigModal } from "@/components/developer/PaymentGatewayConfigModal";
import {
  CreditCard,
  Plus,
  Search,
...
export default function PaymentGatewayIntegrationsPage() {
  ...
  const fetchConfigs = useCallback(async () => {
    ...
    try {
      ...
      showErrorToast(
        err?.message || "Failed to load Payment Gateway configurations"
      );
      setConfigs([]);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this Payment Gateway configuration?"
      )
    )
      return;
    try {
  ...
  const handleToggleActive = async (config: any) => {
    try {
      const res = await api.put(
        `/admin/developer/api-integrations/payment-gateway/${config.id}`,
        {
          ...config,
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      <ApiIntegrationsHeader
        ...
        subtitle="Configure online checkout payment solutions including Razorpay, PhonePe, Stripe, and Cashfree."
        configCount={configs.filter((c) => c.is_active).length}
        secondaryTitle="GATEWAYS ONLINE"
        secondaryCount={configs.length}
        secondarySubtext="Supported merchant accounts"
        dispatchedCount={15420}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Active checkout payment providers &amp; secret credentials
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center space-y-3">
          ...
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            Click &quot;Add Payment Gateway Config&quot; to set up your primary merchant payment processor.
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div>
                  ...
                  <div className="space-y-1.5 text-xs text-slate-600 bg-slate-50/50 p-3 rounded-xl border border-slate-100 font-mono">
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-400 font-sans">Merchant ID:</span>
                      <span className="font-semibold text-slate-700 truncate max-w-[150px]">
                        {data.merchant_id || "N/A"}
                      </span>
                    </div>
                    <div className="flex justify-between">
