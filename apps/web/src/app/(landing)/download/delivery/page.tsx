...
export default function DeliveryDownloadPage() {
  ...
  const fetchVersionInfo = async () => {
    setLoading(true);
    try {
      const res = await api.get<any>("/api/app-version/delivery");
      if (res.data) {
        setData(res.data);
      } else {
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        <nav className="flex items-center gap-1.5 text-xs text-slate-500 mb-8">
          ...
          <ChevronRight size={12} className="text-slate-300" />
          <span className="font-semibold text-slate-800">Delivery Partner App</span>
        </nav>

        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
            <RefreshCw className="animate-spin text-slate-900 mb-4" size={32} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="md:col-span-2 bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-100 flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-start gap-4">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-slate-950 border border-slate-900/10 flex items-center justify-center shadow-inner shrink-0">
                    <span className="text-3xl font-extrabold text-white">DP</span>
                  </div>
                  <div>
                    <h1 className="text-xl sm:text-2xl font-black text-slate-900 leading-tight">F2H Delivery Partner App</h1>
                    <p className="text-xs text-slate-400 mt-0.5">Farm to Home Order Logistics & Dispatch Management</p>
                    
                    <div className="flex flex-wrap items-center gap-2 mt-3">
                      <span className="px-2.5 py-1 rounded-full bg-slate-950 text-white text-[10px] font-bold">
                ...
                <div className="mt-8 border-t border-slate-100 pt-6">
                  <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-3">Release Notes</h3>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col sm:flex-row items-center gap-3">
                <a
                  ...
                  download={`f2h-delivery-v${data.latestVersion}.apk`}
                  className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3.5 bg-slate-950 text-white rounded-2xl text-sm font-bold hover:bg-slate-900 active:scale-95 transition-all shadow-md shadow-slate-950/10 cursor-pointer text-center"
                >
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex flex-col items-center justify-center text-center">
              <div className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-700 mb-4 border border-slate-100/50">
                <QrCode size={20} />
              </div>
              ...
              <div className="flex items-center gap-1.5 mt-6 text-[10px] text-slate-400 font-medium">
                <CheckCircle2 size={13} className="text-slate-700" /> Auto-updates with latest build
              </div>
            </div>
          </div>
        ) : null}
